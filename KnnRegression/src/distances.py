...
def manhattan_distances(X, Y):
    ...
    """
    return np.linalg.norm(X[:, None, :] - Y[None, :, :], ord=1, axis=-1)
    raise NotImplementedError
...
def cosine_distances(X, Y):
    ...
    """
    unity = np.ones((len(X), len(Y)))
    cos_sim = np.dot(X,Y.T)/(np.linalg.norm(X[:, None, :], axis=-1) * np.linalg.norm(Y[None, :, :], axis=-1))
    cos_dist = unity - cos_sim
    return cos_dist
    raise NotImplementedError
