...
class Perceptron():
    ...
    def update_weights(self, X, y):
        ...
        """
        flag = True
        # print(np.matmul(np.array([-6, -2, -5]).T, np.array([1, 2, 3])))
        # print(X.shape[0])
        for i in range(X.shape[0]):
            # print(i)
            # print(f"x = {X[i,:]}")
            # print(f"weights{self.weights.T}")
            # print(f"what is this? {np.matmul(self.weights.T, X[i,:])}")
            # print(f"y somethign: {y[i]}")
            if np.matmul(self.weights.T, X[i,:]) * y[i] <= 0:
                # print(f"is this the problem: {X[i,:] * y[i]}")
    ...
    def predict(self, X):
        ...
        for i in range(X.shape[0]):
            # print(f"Are you the issue? {np.matmul(self.weights.T, X[i,:])}")
            if np.matmul(self.weights.T, X[i,:]) > 0 :
                predictions[i] = 1
            else:
                predictions[i] = -1
        print(self.map_pm1_to_01(predictions))
        # raise NotImplementedError
